...
public class FieldIdentifier {
	...
	Field find(List<Field> fields) {
		...
		if (indexOfLabel == -1) {
			throw new RuntimeException(
					String.format("field [%s] could not be found using match mode [%s]", label, matchMode));
		}
		final int indexOfField = indexOfLabel + skip;
		if (indexOfField >= fields.size()) {
			throw new RuntimeException(String.format(
					"field [%s] at index [%i] plus skip [%i] exceed the number of available fields in the screen [%i]",
					label, indexOfLabel, skip, indexOfField));
		}
	...
	private boolean matches(String expected, String actual) {
		return matchExact(expected, actual) || matchExactAfterTrim(expected, actual) || matchRegex(expected, actual)
				|| matchContains(expected, actual) || matchContainsCI(expected, actual);
	}
	...
	private boolean matchContainsCI(String expected, String actual) {
		expected = expected.toLowerCase();
		actual = actual.toLowerCase();
		return matchMode == MatchMode.CI_CONTAINS && actual.contains(expected);
	}
